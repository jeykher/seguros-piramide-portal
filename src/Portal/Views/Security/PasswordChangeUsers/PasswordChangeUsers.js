...
export default function PasswordChangeUsers(props) {
    ...
    async function getProfiles() {
        const jsonProfiles = await Axios.post(`${process.env.GATSBY_API_URL}/asg-api/dbo/security/get_profiles_change_pwd`)
    ...
    async function onSubmit(dataform, e) {
        ...
        const params = 
        {   p_portal_user_id_to_change_pwd: dataform['p_portal_user_id'],
            p_new_pwd: dataform['p_password'],
        }
        console.log('params: ',params)

        await Axios.post('/dbo/security/change_pwd_to_users', params)

        dialog({
            variant: "info",
            catchOnCancel: false,
            title: "Exito",
            description: "La clave ha sido cambiada satisfactoriamente"
        })   
                    ...
                    <CardBody>                
                        <form onSubmit={handleSubmit(onSubmit)} noValidate>
                            ...
                            <SelectSimpleAutoCompleteWithDataController 
                                ...
                                array={users} 
                                required={true} 
                                noOptionsText={'No existen usuarios asociados al perfil seleccionado'}
                                onChange={([e, value]) => { 
                                    return value ? value["VALUE"] : null
                                }}
                            />
                            <PasswordController objForm={objForm} label="Nueva clave" />
                            <PasswordConfirmController objForm={objForm} label="Confirmar la nueva clave" />
                            <br></br><br></br>
