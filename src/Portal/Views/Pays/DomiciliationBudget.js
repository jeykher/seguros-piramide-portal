...
const DomiciliationBudget = (props) => {
    // SESSIONS
    const userInfo = JSON.parse(sessionStorage.getItem('PROFILE'))
    // PROPS
    const { objBudget, dataInvoice } = props
    // STATES
    // Tipo de Moneda
    ...
    // hooks
    const { getBanks, getTypeAccount, getCustomerInfo, getNumberAccounts, updateNumberAccount } = useDomiciliation()
    // FETCHERS
    ...
    const searchCustomerInfo = async () => {
        if(objBudget?.info[0].BUDGET_ID) {
            const result = await getCustomerInfo(objBudget?.info[0].BUDGET_ID)
            let usersBudget = result.p_insured_summary.insured
            if(usersBudget.length == 1) {
                setPayer({
                    customerType: usersBudget[0].customer_type,
                    cpTipoid: usersBudget[0].tipoid,
                    npNumid: usersBudget[0].numid,
                    cpDvid: usersBudget[0].dvid
                })
            }
            if(usersBudget.length == 2) {
                setPayer({
                    customerType: usersBudget[1].customer_type,
                    cpTipoid: usersBudget[1].tipoid,
                    npNumid: usersBudget[1].numid,
                    cpDvid: usersBudget[1].dvid
                })
            }
        }
    }
    ...
    const handleChangeBankAccount = (e) => {
        ...
        if(inputTypeAccount) {
            searchNumberAccounts({
                cpTipoId: payer.cpTipoid,
                npNumId: payer.npNumid,
                cpDvid: payer.cpDvid,
    ...
    const handleSubmit = (e) => {
        ...
        // TODO: Cambio temporal en la propiedad de cpCodusr de userInfo.PORTAL_USERNAME por un dato hardcore
        const dataForm = {
...
export default DomiciliationBudget
