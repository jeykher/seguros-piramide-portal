...
export default function PlansPropertyCobertSlick({ plans, cobertsDescrip, cobertsProperty, onBeforeChange, sliderRef }) {
  return (
          ...
          <PricingDetails index={index}>
            <ul>
              ...
              {cobertsDescrip.map((cobertdesc, index) => {
                const cobert = cobertsProperty.find(cob => plan.codprod === cob.codprod && plan.codplan === cob.codplan && 
                  plan.revplan === cob.revplan &&  cob.codcobert === cobertdesc.id )
